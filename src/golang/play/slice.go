...
func main() {
	...
	/// copy를 이용한 slice 복사
	source := []int{0, 1, 2}                          // source slice 배열선언 및 초기화
	target := make([]int, len(source), cap(source)*2) // target slice 배열 선언
	copy(target, source)                              // target에 source slice 데이터 복사
	fmt.Println(target)                               // [0 1 2] 출력
	println(len(target), cap(target))                 // 3, 6 출력

	// slice의 구조
	// 1. s := []int{0, 1, 2, 3, 4, 5}    > 배열 len 만큼 cap가 자동설정
	// >> 배열포인터(0) | len(6) | cap(6)
	// 2. s[2:5]			> slice된 길이만큼 len 설정, cap는 2~끝까지 cap 설정
	// >> 배열포인터(2) | len(3) > 2,3,4 | cap(4) > 2,3,4,5
